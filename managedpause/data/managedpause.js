...
Deluge.ux.preferences.ManagedPausePage = Ext.extend(Ext.Panel, {
	
	border: false,
	title: _('Managed Pause'),
	layout: 'fit',
	
	initComponent: function() {
		Deluge.ux.preferences.ManagedPausePage.superclass.initComponent.call(this);
		...
		this.ignoreSeeding = this.options.add({
			xtype: 'checkbox',
			boxLabel: _('Ignore seeding torrents'),
			hideLabel: true,
			name: 'ignore_seeding',
			value: false
		});
	...
	onRender: function(ct, position) {
		Deluge.ux.preferences.ManagedPausePage.superclass.onRender.call(this, ct, position);
	...
	onApply: function() {
		...
		config['ignore_seeding'] = this.ignoreSeeding.getValue();
		
		deluge.client.managedpause.set_config(config);
	},
	
	afterRender: function() {
		Deluge.ux.preferences.ManagedPausePage.superclass.afterRender.call(this);
		this.updateConfig();
	},
	
	updateConfig: function() {
		deluge.client.managedpause.get_config({
			success: function(config) {
				this.schedule.setConfig(config['button_state']);
				this.ignoreSeeding.setValue(config['ignore_seeding']);
			},
			scope: this
		});
	}
...
Deluge.plugins.ManagedPausePlugin = Ext.extend(Deluge.Plugin, {
	...
	onEnable: function() {
		this.prefsPage = deluge.preferences.addPage(new Deluge.ux.preferences.ManagedPausePage());
	}
});

Deluge.registerPlugin('ManagedPause', Deluge.plugins.ManagedPausePlugin);